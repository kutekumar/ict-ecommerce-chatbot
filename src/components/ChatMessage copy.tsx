...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  const parsedProducts = useMemo<Product[]>(() => {
    ...
    // 1) Try JSON parse (webhook might return JSON string)
    try {
    ...
    // 2) DOM parsing (client-side) - preferred approach for HTML responses
    try {
      if (typeof window !== "undefined" && typeof DOMParser !== "undefined") {
        const doc = new DOMParser().parseFromString(message, "text/html");

        // gather candidate elements: <li> first (likely per-product), fallback to <p>
        ...
        for (const el of elements) {
          ...
          // name detection: <strong> or "Product:" patterns
          const strong = el.querySelector("strong");
          const nameFromStrong = strong ? (strong.textContent || "").trim() : null;
          const nameMatch = text.match(/Product[:\s]*([^\n<]+)/i);
          const name = (nameFromStrong && nameFromStrong.replace(/^Product[:\s]*/i, "").trim()) ||
          ...
          // image detection - prefer <img> src
          let img = el.querySelector("img")?.getAttribute("src") || "";
          if (!img) {
            // try markdown image inside innerHTML
          ...
          // anchor detection: prefer anchors with detail/product keywords or href not pointing to images
          const anchors = Array.from(el.querySelectorAll("a"));
          const detail = chooseBestAnchor(anchors as HTMLAnchorElement[], img) || "";

          // Accept entry if we have a name and at least one of price/image/detail
      ...
      // DOMParser not available or parse failed; continue to regex fallback
    }

    // 3) Regex fallback (broad pattern)
    ...
    while ((m = productRegex.exec(message)) !== null) {
      ...
      if (name) {
        // if link is an image link, ignore it for detailLink
      ...
      <div className={`max-w-[80%] ${isUser ? "order-2" : "order-1"}`}>
        ...
        <div
          className={`rounded-2xl px-4 py-3 message-shadow ${
            isUser ? "user-message text-white ml-auto rounded-br-md" : "bot-message text-foreground rounded-bl-md"
          }`}
        >
              ...
              {parsedProducts.map((p, i) => (
                ...
                >
                  {p.image ? (
                    ...
                    <div style={{ width: 125, height: 125 }} className="rounded-md bg-gray-100 mb-3 flex items-center justify-center">
